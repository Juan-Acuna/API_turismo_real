...
namespace Conection
{
    ...
    public class OracleCommandManager
    {
        ...
        public bool NonQueryCommand(String comando)
        {
            try
            {
                var cmd = new OracleCommand(comando, con);
        ...
        public OracleDataReader QueryCommand(String comando)
        {
            var cmd = new OracleCommand(comando, con);
            cmd.CommandType = CommandType.Text;
        ...
        public ProcedureResult ExecuteFunction(String functionName, ProcedureParams prms, OracleDbType returnType)
        {
        ...
        public class ProcedureParams
        {
            ...
            public void AddParam(String paramName, object value, ParameterDirection dir)
            {
                ...
                if (value is int)
                {
                    par.Type = OracleDbType.Int32;
                }
                else if (value is char)
                {
                    par.Type = OracleDbType.Char;
                }
                else if (value is String)
                {
                    par.Type = OracleDbType.Varchar2;
                }
                else if (value is bool)
                {
                    par.Type = OracleDbType.Boolean;
                }
                else if (value is double)
                {
                    par.Type = OracleDbType.Double;
                }
                else
                {
            ...
            public void ParseParams(OracleCommand command)
            {
                if (status.Equals("Finished"))
                {
                    return;
            ...
            private class Param
            {
                ...
                public Param() { }
                public Param(String name, object value, OracleDbType type, ParameterDirection direction)
                {
                    this.Name = name;
                    this.Value = value;
                    this.Type = type;
